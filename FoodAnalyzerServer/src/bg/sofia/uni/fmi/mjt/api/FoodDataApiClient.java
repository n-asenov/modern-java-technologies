...
public class FoodDataApiClient {
    ...
    public List<Food> searchFood(String foodName)
        ...
        FoodSearchResponse foodSearchResponse = gson.fromJson(initialResponse.body(),
    ...
    public BrandedFood getBrandedFood(long brandedFoodId)
            throws InvalidFoodIdException, IOException, InterruptedException {
    ...
    private URI getUri(String foodName, int pageNumber) {
        ...
        uri.append(scheme).append("://").append(host).append("/fdc/v1/search").append('?')
                .append("generalSearchInput=").append(foodName).append("&requireAllWords=true")
                .append("&pageNumber=").append(pageNumber).append("&api_key=").append(apiKey);
    ...
    private URI getUri(long foodId) {
        ...
        uri.append(scheme).append("://").append(host).append("/fdc/v1/").append(foodId).append('?')
    ...
    private List<Food> getFoodsFromCurrentSearch(String foodName, FoodSearchResponse response) {
        ...
        for (int currentPage = 1; currentPage <= 200
                && currentPage <= response.getTotalPages(); currentPage++) {
    ...
    private String getFoodDetailsById(long foodId)
            throws InvalidFoodIdException, IOException, InterruptedException {
